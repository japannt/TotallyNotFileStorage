    ...
    function CreateTimeGUI()
    	...
    	CreateDropdown("Box Style", 10, 30, {"2D", "2D Corners", "3D"}, "esp_player_box_mode", visplayer)
    	CreateCheckBox("Name ESP", 10, 75, "esp_player_name", true, visplayer)
    	CreateCheckBox("HP ESP", 10, 95, "esp_player_hp", true, visplayer)
    	CreateCheckBox("Armor ESP", 10, 115, "esp_player_armor", true, visplayer)
    	CreateCheckBox("Weapon ESP", 10, 135, "esp_player_weapon", true, visplayer)
    	CreateCheckBox("Fancy Weapon Name", 10, 155, "esp_player_weapon_fancy", false, visplayer)
    	CreateCheckBox("Rank ESP", 10, 175, "esp_player_rank", true, visplayer)
    	CreateCheckBox("Team ESP", 10, 195, "esp_player_team", false, visplayer)
    	CreateCheckBox("Skeleton ESP", 10, 215, "esp_player_skeleton", true, visplayer)
    	CreateCheckBox("Snaplines", 10, 235, "esp_player_snaplines", true, visplayer)
    	CreateCheckBox("Glow ESP", 10, 255, "esp_player_glow", true, visplayer)
    	CreateCheckBox("Player Chams", 10, 275, "esp_player_chams", true, visplayer)
    	CreateCheckBox("Player Chams Overlay", 10, 295, "esp_player_chams_overlay", true, visplayer)
    	CreateCheckBox("Player Chams XYZ", 10, 315, "esp_player_chams_xyz", true, visplayer)
    	CreateButton("Cham Material", "Opens the cham material selector.", CreateMaterialList, 10, 335, visplayer)
    	CreateCheckBox("Hitbox ESP", 10, 360, "esp_player_hitbox", true, visplayer)
    	CreateSlider("ESP Max Render Distance", 10, 380, "esp_player_render_distance", 500, 10000, 0, visplayer)
    	CreateCheckBox("Hide Dormant Players", 10, 420, "esp_player_dormant", false, visplayer)
    	CreateCheckBox("Highlight Friends Box", 10, 440, "esp_player_highlight_box", true, visplayer)
    	CreateCheckBox("Highlight Friends Name", 10, 460, "esp_player_highlight_name", true, visplayer)
    	...
    	CreateSlider("Bloom Scale", 10, 270, "esp_other_bloom", 0, 1, 2, visother)
    	CreateCheckBox("Hitsound", 10, 310, "esp_other_hitsound", false, visother)
    	CreateCheckBox("Draw Aimbot FOV", 10, 330, "esp_other_drawfov", true, visother)
    	CreateCheckBox("Fullbright", 10, 350, "esp_other_fullbright", false, visother)
    	CreateCheckBox("Thirdperson", 10, 370, "esp_other_thirdperson", false, visother)
    	CreateKeybind(100, 370, "thirdperson_key", visother)
    	CreateSlider("Thirdperson Distance", 10, 390, "esp_other_thirdperson_distance", 5, 50, 0, visother)
    	CreateCheckBox("Freecam", 10, 430, "esp_other_freecam", false, visother)
    	CreateKeybind(100, 430, "freecam_key", visother)
    	CreateSlider("Freecam Speed", 10, 450, "esp_other_freecam_speed", 1, 10, 0, visother)
    	CreateButton("Entity List", "Open the entity list menu.", CreateEntityList, 10, 495, visother)
    	...
    	CreateCheckBox("Override FOV", 10, 240, "misc_fov", false, misc)
    	CreateSlider("FOV Value", 10, 260, "misc_fov_value", 50, 160, 0, misc)
    	CreateCheckBox("Viewmodel Changer", 10, 300, "misc_viewmodel", false, misc)
    	CreateCheckBox("Remove Bob", 10, 320, "misc_bob", false, misc)
    	CreateCheckBox("Remove Sway", 10, 340, "misc_sway", false, misc)
    	CreateSlider("Viewmodel X", 10, 360, "misc_vm_x", -50, 50, 0, misc)
    	CreateSlider("Viewmodel Y", 10, 400, "misc_vm_y", -30, 30, 0, misc)
    	CreateSlider("Viewmodel Z", 10, 440, "misc_vm_z", -20, 20, 0, misc)
    	CreateSlider("Viewmodel Pitch", 10, 480, "misc_vm_p", -90, 90, 0, misc)
    	CreateSlider("Viewmodel Yaw", 10, 520, "misc_vm_ya", -90, 90, 0, misc)
    	CreateSlider("Viewmodel Roll", 10, 560, "misc_vm_r", -90, 90, 0, misc)
    	...
    	CreateDropdown("Name Font", 10, 265, CheatFonts, "name_font", config)
    	CreateSlider("Name Font Size", 10, 305, "name_font_size", 10, 20, 0, config)
    	CreateDropdown("Flag Font", 10, 345, CheatFonts, "flag_font", config)
    	CreateSlider("Flag Font Size", 10, 385, "flags_font_size", 10, 20, 0, config)
    	CreateButton("ESP Mode " .. GetRenderMode(), "If your ESP is not working on a server try changing this to unsafe. (THIS SHOULD ALWAYS BE PROTECTED WHEN POSSIBLE TO MAXIMIZE SCREENGRAB PROTECTION)", SwapRender, 10, 425, config)
    	        ...
    	        if IsValid(v) && v:IsTerror() && !v:IsDetective() && !v.Traitor && v != LocalPlayer() then
    	            for _, w in ipairs(v:GetWeapons()) do
    	            	if w and w.CanBuy and table.HasValue(w.CanBuy, ROLE_TRAITOR) && w.AutoSpawnable == false then
    	            		v.Traitor = true
    	            		chat.AddText(Color(61, 149, 217), "[Time's Hack] ", Color(222, 222, 222), v:Nick(),  " is a ", Color(255,0,0), "TRAITOR", Color(222, 222, 222), " with a ", Color(0, 255, 0), w:GetClass() .. "!")
    	            	end
    	            end
    	        end
    	    end
